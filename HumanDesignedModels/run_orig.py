...
def main():
    ...
    for epoch in range(config.epochs):
        train(model, optimizer, train_loader, epoch, train_data.get_class_weights())
    ...
    logger.info("Final best Accuracy = {:.4%}".format(best_wa))


def train(model, optimizer, train_loader, epoch, class_weights):
    ...
    for step, (trn_x, trn_y) in enumerate(train_loader):
        ...
        wa, f1, r, p = utils.scores(output, trn_y, class_weights)
        ...
        #writer.add_scalar('train/wa', weighted_accuracy.avg, cur_step)
        cur_step += 1

    logger.info("Train: [{:2d}/{}] Final Acc {:.4%} Train WA {:.4%}".format(epoch + 1, config.epochs, accuracy.avg, weighted_accuracy.avg))
...
def validate(valid_loader, model, epoch, cur_step, class_weights):
    ...
    with (torch.no_grad()):
        for step, (X, y) in enumerate(valid_loader):
            ...
            loss = torch.nn.functional.cross_entropy(logits, y.argmax(dim=1), weight=class_weights.to(device))
    ...
    logger.info(
        "Valid: [{:2d}/{}] Final Acc {:.4%} Final W. Acc {:.4%}".format(epoch + 1, config.epochs, accuracy.avg, weighted_accuracy.avg))
